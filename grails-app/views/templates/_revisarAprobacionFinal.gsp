...
<elm:container tipo="horizontal" titulo="Revisión de nota de pedido">
    ...
    <div class="btn-toolbar toolbar" style="margin-top: 10px">
        <div class="btn-group">
            <g:link controller="notaPedido" action="lista${tipo}" class="btn btn-default">
...
<script type="text/javascript">
    ...
    function submitAprobar() {
        if ($("#frmAprobar").valid()) {
            ...
            $.ajax({
                type    : "POST",
                url     : "${createLink(controller:'notaPedido', action:'aprobarFinal_ajax')}",
                data    : {
                    id   : "${nota.id}",
                    auth : $("#auth").val(),
                    cot  : $("#cot").val(),
                ...
                success : function (msg) {
                    ...
                    if (parts[0] == "SUCCESS") {
                        setTimeout(function () {
                            location.href = "${createLink(conroller:'notaDePedido', action:'lista'+tipo)}";
                        }, 1000);
                    } else {
                        closeLoader();
                    }
    ...
    function submitCotizaciones() {
        if ($("#frmCotizaciones").valid()) {
            ...
            $.ajax({
                type    : "POST",
                url     : "${createLink(controller:'notaPedido', action:'solicitarCotizaciones_ajax')}",
                data    : {
                    id   : "${nota.id}",
                    auth : $("#auth").val(),
                    obs  : $("#obs").val()
    ...
    function submitNegar() {
        if ($("#frmNegar").valid()) {
            ...
            $.ajax({
                type    : "POST",
                url     : "${createLink(controller:'notaPedido', action:'negarFinal_ajax')}",
                data    : {
                    id    : "${nota.id}",
                    auth  : $("#auth").val(),
                    razon : $("#razon").val()
    ...
    function submitBodega() {
        if ($("#frmBodega").valid()) {
            ...
            $.ajax({
                type    : "POST",
                url     : "${createLink(controller:'notaPedido', action:'bodegaFinal_ajax')}",
    ...
    $(function () {
        $("#btnAprobar").click(function () {
            $.ajax({
                type    : "POST",
                url     : "${createLink(controller:'notaPedido', action:'dlgAprobarFinal_ajax')}",
                data    : {
                    id : "${nota.id}"
                },
                success : function (msg) {
        ...
        $("#btnCots").click(function () {
            $.ajax({
                type    : "POST",
                url     : "${createLink(controller:'notaPedido', action:'dlgSolicitarCotizaciones_ajax')}",
                data    : {
                    id : "${nota.id}"
                },
                success : function (msg) {
                    var b = bootbox.dialog({
                        id      : "dlgSolicitarCots",
                        title   : "Solicitar cotizaciones para Nota de Pedido",
                        message : msg,
                        buttons : {
                            guardar  : {
        ...
        $("#btnNegar").click(function () {
            $.ajax({
                type    : "POST",
                url     : "${createLink(controller:'notaPedido', action:'dlgNegarFinal_ajax')}",
                data    : {
                    id : "${nota.id}"
                },
                success : function (msg) {
